...
// Secret JWT
const JWT_SECRET = "votre_secret_key_super_securise_12345"; // À mettre en env variable en prod
...
// Vérifier si token est valide (pour la persistence de session)
...
app.post("/api/posts/:id/share", (req, res) => {
  ...
  db.run(
    ...
    [req.params.id],
    () => {
      // Créer une nouvelle publication partagée
      db.get(
...
app.get("/api/admin/messages", (req, res) => {
  db.all(
    `SELECT m.id, m.senderId, m.receiverId, m.text, m.timestamp,
            u1.name as senderName, u1.email as senderEmail,
            u2.name as receiverName, u2.email as receiverEmail
     FROM messages m
     JOIN users u1 ON m.senderId = u1.id
     JOIN users u2 ON m.receiverId = u2.id
     ORDER BY m.timestamp DESC`,
